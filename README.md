...
Build the migration(s) in Knex inside the `data/migrations` folder using appropriate data types and constraints. **You must use the table names and the column names described below.** To give a primary key a name different from `id`, do `table.increments("project_id")` instead of `table.increments()`.
...
SQL is the language used to write relational databases
2. Why do tables need a Primary Key?
Each record in the table needs a unique identifier to be able to access one record at a time if necessary
3. What is the name given to a table column that references the Primary Key on another table?
Foreign key
4. What do we need in order to have a _many to many_ relationship between two tables?
We need an assignment (aka intermediary) table to link foreign keys from multiple tables together in unique combinations.
